...
fn callback_selector_body(
    ...
    quote! {
        let mut ___call_result_loader___ = dharitri_wasm::io::EndpointDynArgLoader::<Self::Api>::new();
        ...
        .filter_map(|m| {
            if let PublicRole::Callback(callback) = &m.public_role {
                ...
                let call_result_assert_no_more_args = if has_call_result {
                    quote! {
                        dharitri_wasm::io::assert_no_more_args::<Self::Api, _>(&___call_result_loader___);
                    }
                } else {
                    quote! {}
                };
                ...
                let match_arm = quote! {
                        ...
                        #(#arg_init_snippets)*
                        dharitri_wasm::io::assert_no_more_args::<Self::Api, _>(&___cb_arg_loader___);
