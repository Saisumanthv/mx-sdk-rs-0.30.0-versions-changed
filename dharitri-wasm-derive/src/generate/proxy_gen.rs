...
pub fn generate_proxy_endpoint_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> dharitri_wasm::types::ContractCall<Self::Api, <#ret_tok as dharitri_wasm::dharitri_codec::TopEncodeMulti>::DecodeAs>
        #generics_where
    };
    result
}
...
pub fn generate_proxy_deploy(init_method: &Method) -> proc_macro2::TokenStream {
    ...
    let sig = quote! {
        ...
        #msig {
            let ___address___ = self.into_fields();
            let mut ___contract_deploy___ = dharitri_wasm::types::new_contract_deploy::<Self::Api>(
                ___address___,
            );
            #(#arg_push_snippets)*
