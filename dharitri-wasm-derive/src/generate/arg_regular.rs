    ...
    match &arg.ty {
        syn::Type::Reference(type_reference) => {
            ...
            if let syn::Type::Slice(slice_type) = &*type_reference.elem {
                ...
                quote! {
                    dharitri_wasm::load_single_arg::<Self::Api, Box<[#slice_elem]>>(#arg_index_expr, #arg_name_expr)
                }
            } else {
                ...
                if let syn::Type::Path(syn::TypePath { path, .. }) = referenced_type {
                    if let Some(ident) = path.get_ident() {
                        if *ident == "str" {
                            ...
                            return quote! {
                                dharitri_wasm::load_single_arg::<Self::Api, Box<str>>(#arg_index_expr, #arg_name_expr)
                            };
                        }
                    }
                }
                ...
                quote! {
                    dharitri_wasm::load_single_arg::<Self::Api, #referenced_type>(#arg_index_expr, #arg_name_expr)
                }
            }
        },
        _ => {
            quote! {
                dharitri_wasm::load_single_arg::<Self::Api, #arg_ty>(#arg_index_expr, #arg_name_expr)
            }
        },
    }
}
...
pub fn generate_load_dyn_arg(
    ...
    match &arg.ty {
        syn::Type::Reference(type_reference) => {
            ...
            let referenced_type = &*type_reference.elem;
            quote! {
                let #pat: & #referenced_type = &dharitri_wasm::load_dyn_arg::<Self::Api, _, _>(#loader_expr, #arg_name_expr);
            }
        },
        _ => {
            quote! {
                let #pat: #arg_ty = dharitri_wasm::load_dyn_arg::<Self::Api, _, _>(#loader_expr, #arg_name_expr);
            }
        },
    }
}
