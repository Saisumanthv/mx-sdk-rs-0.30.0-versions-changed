...
pub trait PauseProxy {
    ...
    fn add_contracts(&self, #[var_args] contracts: MultiValueEncoded<ManagedAddress>) {
        self.require_owner();
        self.contracts().extend(contracts);
    }

    #[endpoint(removeContracts)]
    fn remove_contracts(&self, #[var_args] contracts: MultiValueEncoded<ManagedAddress>) {
        self.require_owner();
        self.contracts().remove_all(contracts);
    }

    #[endpoint(addOwners)]
    fn add_owners(&self, #[var_args] owners: MultiValueEncoded<ManagedAddress>) {
        self.require_owner();
        self.owners().extend(owners);
    }

    #[endpoint(removeOwners)]
    fn remove_owners(&self, #[var_args] owners: MultiValueEncoded<ManagedAddress>) {
        self.require_owner();
